...
async function generateMarkdownContent() {
  try {
    const markdownPath = path.join(process.cwd(), 'docs', 'user-guide.md');
    const markdownContent = fs.readFileSync(markdownPath, 'utf8');
    return marked(markdownContent);
  } catch (error) {
    console.error('Error al leer el archivo Markdown:', error);
    return '<p>No se pudo cargar el contenido de la guía de usuario.</p>';
  }
}
    ...
    <Container maxWidth="md" sx={{ py: 4 }}>
      <Box mb={4}>
        <Breadcrumbs aria-label="breadcrumb">
          ...
          <Typography color="text.primary">Guía de Usuario</Typography>
        </Breadcrumbs>
      </Box>
